...
const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  ...
  try {
    ...
    console.log('- MerchantRequestID:', payload.MerchantRequestID)
    console.log('- CheckoutRequestID:', payload.CheckoutRequestID)
    console.log('- TransactionReference:', payload.TransactionReference)
    console.log('- Msisdn:', payload.Msisdn)
    ...
    try {
      ...
      if (transaction) {
        ...
        console.error('- MerchantRequestID:', MerchantRequestID)
        console.error('- CheckoutRequestID:', CheckoutRequestID)
        console.error('- TransactionReference:', TransactionReference)
        console.error('- Msisdn:', Msisdn)
        
        // List all pending transactions to debug
        ...
        console.error('Recent pending transactions:', allPending)
      }
    } catch (dbErr) {
